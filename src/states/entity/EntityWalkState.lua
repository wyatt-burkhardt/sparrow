...
function EntityWalkState:init(entity, dungeon)
    ...
    -- keep track on if we hit a wall
    self.bumped = false
end

function EntityWalkState:update(dt)
    --assume we havent hit a wall
    self.bumped = false

    if self.entity.direction == 'left' then
        self.entity.x = self.entity.x - self.entity.walkSpeed * dt

        -- if we hit the edge of map return entity to the map and bump them
...
function EntityWalkState:processAI(params, dt)
    ...
    if self.moveDuration == 0 or self.bumped then
        ...
        -- set initial move duration and direction
